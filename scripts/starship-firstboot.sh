...
_enable_fleet_bus() {
  ...
  chmod 600 /etc/starship/nats-token 2>/dev/null || true
  ...
  if [[ -f "$conf_src" ]]; then
    ...
    chmod 600 /etc/starship/nats/fleet-bus.active.conf 2>/dev/null || true
    chown nats:nats /etc/starship/nats/fleet-bus.active.conf 2>/dev/null || true
  else
...
EOF
  chmod 600 /etc/starship/nats.env 2>/dev/null || true
...
_enable_accounts_bus() {
  ...
  if [[ -f "$out/fleet-accounts.conf" ]]; then
    ...
    chmod 600 "$out/fleet-accounts.conf" 2>/dev/null || true
    chown nats:nats "$out/fleet-accounts.conf" 2>/dev/null || true
  else
...
fixer_cmd="${REPO_DIR}/scripts/fix-nats-secret-modes.sh"
if [[ ! -f "$fixer_cmd" && -f /opt/starship/lib/starship/scripts/fix-nats-secret-modes.sh ]]; then
  fixer_cmd=/opt/starship/lib/starship/scripts/fix-nats-secret-modes.sh
fi
if [[ -f "$fixer_cmd" ]]; then
  bash "$fixer_cmd" 2>/dev/null || echo "warn: fix-nats-secret-modes failed" >&2
fi
