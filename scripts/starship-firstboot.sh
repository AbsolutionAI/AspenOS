...
# 4b) NATS bus selection — accounts (default, all profiles) vs fleet token
# H-001 (ASP-169): no-auth agent-bus mode removed; bus always authenticates.
_install_nats_configs() {
  ...
  cp "$nats_src/fleet-accounts.conf.tmpl" /etc/starship/nats/ 2>/dev/null || true
...
_enable_fleet_bus() {
  ...
  if [[ -f "$conf_src" ]]; then
    ...
    echo "ERROR: fleet-bus.conf missing and STARSHIP_NATS_MODE=fleet was forced." >&2
    echo "       Refusing to start an unauthenticated bus (H-001). Use accounts mode instead." >&2
    return 1
  fi
...
_enable_accounts_bus() {
  ...
  if [[ -f "$gen" ]]; then
    ...
    echo "warn: gen-nats-accounts.sh not found — cannot generate account creds" >&2
  fi
  if [[ -f "$out/fleet-accounts.conf" ]]; then
    ln -sfn "$out/fleet-accounts.conf" /etc/starship/nats/active.conf
  else
    echo "ERROR: fleet-accounts.conf could not be materialized (H-001)." >&2
    echo "       Refusing to continue without an authenticated bus config." >&2
    return 1
  fi
...
# Bus selection (H-001):
#   STARSHIP_NATS_MODE=fleet (or STARSHIP_FLEET_BUS=1) → shared-token fleet bus
#   anything else (default) → multi-tenant accounts + nkeys (OPS/EDGE/RANGE/TELEM)
# There is no no-auth path anymore.
if [[ "${STARSHIP_NATS_MODE:-}" == "fleet" || "${ENABLE_FLEET_BUS:-}" == "1" || "${ENABLE_FLEET_BUS:-}" == "true" ]]; then
  _enable_fleet_bus
else
  _enable_accounts_bus
fi
