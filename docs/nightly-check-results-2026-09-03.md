# Nightly Check Results — 2026-09-03
...
- Branch: `master`
- Last commit: `f16656e` — `fix(nightly): include packaging/*.sh in shell syntax check`

## Static inventory
- systemd unit files: 9 in `systemd/`, matches baseline
...
None. Baseline matches: 101 passed, 1 known failure (C11 p50).

## Shell syntax coverage
35 scripts (34 in `scripts/`, 1 in `packaging/`), all pass `bash -n`. The `packaging/install-starship.sh` is now included (added in ASP-546 fix).