...
| Hermes ingestion hook | `agents/agent_daemon.py` `ingest_memory_record()` — writes a `source="hermes"` ingest record per processed command, best-effort/failure-isolated (BEL-154 Component 1 agent-side wiring) |
| OpenCode ingestion hook | `agents/opencode_memory.py` `ingest_opencode_record()` + CLI — writes a `source="opencode"` ingest record per task/session (files/tools/refs metadata, content truncation, failure-isolated); invocable as a hook/wrapper (BEL-154 Component 1 agent-side wiring) |
| Aider ingestion hook | `agents/aider_memory.py` `ingest_aider_record()` + CLI — writes a `source="aider"` ingest record per edit/session (files/tools/refs metadata, content truncation, failure-isolated); invocable as a post_edit hook/wrapper (BEL-154 Component 1 agent-side wiring) |
| Paperclip ingestion hook | `agents/paperclip_memory.py` `ingest_paperclip_record()` + CLI — writes a `source="paperclip"` ingest record per task/session (files/tools/refs metadata, content truncation, failure-isolated); invocable as a run-completion hook/wrapper (BEL-154 Component 1 agent-side wiring) |
