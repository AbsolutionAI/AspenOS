# Nightly Packaging & Deployment Check — 2026-09-24 08:00 UTC

| Field | Value |
|-------|--------|
| **Verdict** | **PASS** (within known baseline) |
| **When** | 2026-09-24 08:00 UTC |
| **Host** | bt-asp-srv control plane |
| **Repo tip** | `77fc673` (`feat(safety): H-023 independent estop watchdog (ASP-433)`) |
| **Version** | 2.2.0 |
...
| **Duration** | 18613 ms |
| **nats-server** | v2.14.5 |
| **Issue** | [ASP-644](/ASP/issues/ASP-644) |
| **Executor** | Opencode |
...
| Python test suite (`pytest tests/`) | **466 passed / 4 skipped, 0 failed** |
...
| 13 Python test suite | 4 | PASS (466 passed / 4 skipped) |
...
| **Total** | **150** | **149 PASS / 1 FAIL (known)** |

## Static inventory

| Item | Value |
|-------|--------|
| systemd unit files | **9** (in `systemd/`, 9 mirrored in `dist/pkgroot/lib/systemd/system/`) |
...
**C11 sandbox p50 under 2ms** — ADR 0001 threshold. Measured **3.703 ms** on this run
(prior runs 3.645 ms, 3.462 ms, 3.481 ms, 3.428 ms, 3.181 ms). Hardware-dependent; documented
in the runbook. Not a packaging regression.

## Deviations from baseline doc

- **Python suite count grew 429 → 466 (>150 floor)** (+37): new tests landed with
  `feat(gatekeeper): H-022 HITL vault approval gate (ASP-432)` and
  `feat(safety): H-023 independent estop watchdog (ASP-433)` since the last nightly.
  Still **4 skipped (optional deps: aiohttp, mcp.server), 0 failures**.
- No other section changed; total check count stays **150**.

## Repo changes in this run

- `docs/ops/nightly-results-2026-09-24T08-00.md` — this results file.
- `docs/ops/NIGHTLY_PACKAGING_DEPLOY_CHECK.md` — baseline refresh (Python suite 429 → 466 passed).

## Findings

1. **C11 p50 benchmark** — 3.703 ms, above the 2 ms ADR 0001 threshold. Consistent with
   prior runs (3.645 ms on 2026-09-23T08:00, 3.462 ms on 2026-09-22T08:00, 3.481 ms on
   2026-09-22T02:00, 3.181 ms on 2026-09-21); not actionable without hardware change or
   sandbox optimization.
2. **No packaging regressions** — all 22 sections pass structurally. Deb metadata/units,
   ISO structure, Windows packaging, and all security gates
   (H-010/H-019/F-009/F-011/F-012/F-013/F-014) green.
3. **Baseline drift (benign, additive)** — Python suite grew 429 → 466 passed via new
   H-022 (ASP-432) and H-023 (ASP-433) fixture tests. No skips/failures regressed.