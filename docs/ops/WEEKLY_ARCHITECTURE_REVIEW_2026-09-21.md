...
| D3 | Gatekeeper packaging placement | Edge-adjacent plugin vs core-edge binary; durable token store later | Medium |
...
1. **Gatekeeper production packaging placement (ADR-0004/0008/0009 residual)** — choose monorepo shim → plugin vs core-edge binary; durable token backend later. Flash plan-only until packaging sprint.  
