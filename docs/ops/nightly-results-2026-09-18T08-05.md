# Nightly Packaging & Deployment Check — 2026-09-18 08:05 UTC

| Field | Value |
|-------|--------|
| **Verdict** | **PASS** (within known baseline) |
| **When** | 2026-09-18 08:05 UTC |
| **Host** | bt-asp-srv control plane |
| **Repo tip** | `0e958dc` (`feat(sentinel): publish aspen.sentinel.fleet.overview producer (ASP-597)`) |
| **Version** | 2.2.0 |
...
| **Duration** | 15116 ms |
| **nats-server** | v2.14.5 |
| **Issue** | [ASP-615](/ASP/issues/ASP-615) |
...
| Python test suite (`pytest tests/`) | **335 passed, 4 skipped** (optional deps aiohttp, mcp.server), **0 failures** |
...
- **Python test suite count rose 316 → 335 passed** (4 skipped, 0 failures) — growth from sentinel fleet-overview tests and other additions since [ASP-610](/ASP/issues/ASP-610). Comfortably above the ≥150 gate.
...
Full stdout retained on run scratch for ASP-615 heartbeat (`nightly-check.log`, plus `make-smoke.log`, `iso-smoke.log`, `pytest-q.log`).