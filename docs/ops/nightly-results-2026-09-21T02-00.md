# Nightly Packaging & Deployment Check — 2026-09-21 02:00 UTC
...
| **Repo tip** | `0848280` (`feat(security): fail-open tool-anomaly detector over audit events (F-015/H-015, ASP-379)`) |
| **Version** | 2.2.0 |
...
| **Duration** | 19127 ms |
| **nats-server** | v2.14.5 |
| **Issue** | [ASP-626](/ASP/issues/ASP-626) |
...
**C11 sandbox p50 under 2ms** — ADR 0001 threshold. Measured **3.181 ms** on this run
(prior runs 3.4–3.5 ms). Hardware-dependent; documented in the runbook. Not a packaging regression.

## Deviations from baseline doc

- Baseline doc still said "90 checks across 20 sections"; suite has since grown to **149 checks across 22 sections** (sections 21/22 from ASP-377/ASP-378 and new smoke/bash checks). Baseline table updated in this run.
- `make iso-smoke` separately **32 pass / 0 fail** (embedded in section 4 as the iso-firstboot smoke check).
- Python count rose from 338 to **418 passed** (4 skipped) as the suite gained tests since the last baseline refresh.

## Repo changes in this run

- `docs/ops/NIGHTLY_PACKAGING_DEPLOY_CHECK.md` — baseline table refreshed to current check counts (149 checks / 22 sections, smoke 62 tests, pytest 418, shell syntax 43).
- This results file landed as a commit.

## Findings

1. **C11 p50 benchmark** — 3.181 ms, above the 2 ms ADR 0001 threshold. Consistent with prior runs; not actionable without hardware change or sandbox optimization.
