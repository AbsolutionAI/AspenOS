# Nightly Packaging & Deployment Check — 2026-09-23 08:04 UTC

| Field | Value |
|-------|--------|
| **Verdict** | **PASS** (within known baseline) |
| **When** | 2026-09-23 08:04 UTC |
| **Host** | bt-asp-srv control plane |
| **Repo tip** | `b563367` (`docs(ops): record nightly 2026-09-22T08:00 results (149/150, only C11 p50 known dev)`) |
| **Version** | 2.2.0 |
...
| **Duration** | 19150 ms |
| **nats-server** | v2.14.5 |
| **Issue** | [ASP-639](/ASP/issues/ASP-639) |
| **Executor** | Opencode |
...
| Python test suite (`pytest tests/`) | **429 passed / 4 skipped, 0 failed** |
...
| 13 Python test suite | 4 | PASS (429 passed / 4 skipped) |
...
**C11 sandbox p50 under 2ms** — ADR 0001 threshold. Measured **3.645 ms** on this run
(prior runs 3.462 ms, 3.481 ms, 3.428 ms, 3.181 ms). Hardware-dependent; documented
in the runbook. Not a packaging regression.

## Deviations from baseline doc

- **Shell syntax coverage grew 43 → 44**: `scripts/` gained `apply-auditor-baseline.sh`
  (feat(security), ASP-370). All 44 pass `bash -n`. Baseline rows 7 and 13 updated.
- **Total check count grew 149 → 150** as a result; no other section changed.
- Python suite stayed at **429 passed / 4 skipped** (unchanged from updated baseline).

## Repo changes in this run

- `docs/ops/nightly-results-2026-09-23T08-04.md` — this results file.
- `docs/ops/NIGHTLY_PACKAGING_DEPLOY_CHECK.md` — baseline refresh (check count 149 → 150, shell syntax coverage 43 → 44).

## Findings

1. **C11 p50 benchmark** — 3.645 ms, above the 2 ms ADR 0001 threshold. Consistent with
   prior runs (3.462 ms on 2026-09-22T08:00, 3.481 ms on 2026-09-22T02:00, 3.181 ms on
   2026-09-21); not actionable without hardware change or sandbox optimization.
...
3. **Baseline drift (benign)** — shell syntax coverage 43 → 44 via the new
   `apply-auditor-baseline.sh` (ASP-370). Total check count 149 → 150.