# Nightly Packaging & Deployment Check — 2026-09-22 08:00 UTC

| Field | Value |
|-------|--------|
| **Verdict** | **PASS** (within known baseline) |
| **When** | 2026-09-22 08:00 UTC |
| **Host** | bt-asp-srv control plane |
| **Repo tip** | `063f831` (`docs(ops): record nightly 2026-09-22T02:00 results (autoinstall refdog, 105/108, 3 cosmetic)`) |
| **Version** | 2.2.0 |
...
| **Duration** | 68657 ms |
| **nats-server** | v2.14.5 |
| **Issue** | [ASP-636](/ASP/issues/ASP-636) |
| **Executor** | Opencode |
...
**C11 sandbox p50 under 2ms** — ADR 0001 threshold. Measured **3.462 ms** on this run
...
- Python test suite grew from **418 → 429 passed** (same 4 skipped: optional deps aiohttp/mcp.server). Monotonic test growth, zero failures — no baseline regression. Baseline row 13 updated.

## Repo changes in this run

- `docs/ops/nightly-results-2026-09-22T08-00.md` — this results file.
- `docs/ops/NIGHTLY_PACKAGING_DEPLOY_CHECK.md` — baseline refresh (Python suite count 418 → 429).

## Findings

1. **C11 p50 benchmark** — 3.462 ms, above the 2 ms ADR 0001 threshold. Consistent with prior runs (3.481 ms on 2026-09-22T02:00, 3.181 ms on 2026-09-21, 3.428 ms on 2026-09-19); not actionable without hardware change or sandbox optimization.
2. **No packaging regressions** — all 22 sections pass structurally. Deb metadata/units, ISO structure, Windows packaging, and all security gates (H-010/H-019/F-009/F-011/F-012/F-013/F-014) green.
3. **Python suite growth** — 429 passed (11 new tests since prior baseline), 4 skipped optional-dep tests, 0 failures.