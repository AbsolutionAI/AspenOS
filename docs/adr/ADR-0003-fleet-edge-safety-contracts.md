...
| `aspen.edge.<node_id>.authorize` | dual-human authz for held proposals | `{proposal_id, human_id, note?}` (H-016 / G8) |
| `aspen.edge.<node_id>.command` | RRM → micro-agent commands | |
| `aspen.safety.estop` | reason, source, ts | **retain + fanout; all RRMs** |
| `aspen.safety.authorize_clear` | one principal toward dual-clear | `{human_id}` (G7/G8) |
...
- Safety-adjacent `propose_act` holds until **two distinct humans** authorize
  (`aspen.edge.<node>.authorize`); act verifies the two-principal record in-window
  (see `docs/security/ACT_GATE_CONTRACT.md`). Single-principal / self-approval /
  duplicate-principal must refuse with stable audited reasons.
- Estop clear is dual-human: two `authorize_clear` then `clear`; bare clear never unlatches.
